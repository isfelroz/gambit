    ...
    return (S) => {
        return S.list()
            ...
            .items([
                ...
                S.documentTypeListItem('page404').child(S.editor().id('page404').schemaType('page404').documentId('page404')),
                S.divider(),
                S.documentTypeListItem('header').child(S.editor().id('header').schemaType('header').documentId('header')),
                S.documentTypeListItem('footer').child(S.editor().id('footer').schemaType('footer').documentId('footer')),
                S.divider(),
                S.documentTypeListItem('settings').child(S.editor().id('settings').schemaType('settings').documentId('settings')),
                S.divider(),
                S.documentTypeListItem('sitemap').child(S.editor().id('sitemap').schemaType('sitemap').documentId('sitemap')),
                S.documentTypeListItem('redirections').child(S.editor().id('redirections').schemaType('redirections').documentId('redirections')),
                // S.listItem()
                //     .title('Settings')
                //     .child(
                //         S.list()
...
export const pageStructure = (arrayStructure: WebStructure[]): StructureResolver => {
    return (S) => {
        const items: (Divider | ListItemBuilder)[] = arrayStructure.map(({ schemaType, singleton = false, definition = null }) => {
            ...
            return S.listItem()
                ...
                .child(S.editor().id(definition.name).schemaType(definition.name).documentId(definition.name))
        })
